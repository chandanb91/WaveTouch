...
//  WaveTouch
//
//  Created by Chandan Balachandra on 05/07/2016.
...
//    let temp = UnsafeMutablePointer<Float>.alloc(x.count)
//    for  i in (0..<x.count) {
//        temp[i] = x[i];
//    }
//    let count = UnsafeMutablePointer<Int32>.alloc(1)
...
class FFT{
    ...
    //let vcObject = ViewController()
    
    
    
//    func fpsData() -> (audioSize:Int, audioDuration:Int) {
//        let size = vcObject.audioRawDataLength()
//        let length = vcObject.audioDuration()
//        let duration = vDSP_Length(floor(log2((length))))
//        return (audioSize: size, audioDuration:Int(duration))
//    }
    
    private func getFrequencies(N: Int, fps:Float) -> [Float] {
    
        // Create an Array with the Frequencies
        //let fps : Float = Float(fpsData().0/fpsData().1)
        let freqs = (0..<N/2).map {
        fps/Float(N)*Float($0)
        }
    ...
    private func generateBandPassFilter(freqs: [Float]) -> ([Float], Int, Int) {
    
        var minIdx = freqs.count+1
        var maxIdx = -1
    
        let bandPassFilter: [Float] = freqs.enumerate().map {
        (index, element) in
    ...
    let lowerFreq : Float = 1
    let higherFreq: Float = 100
    let fps: Float = 256
    let pi = Float(M_PI)
    ...
    func calculate(_input: [Float]) -> [Float] {
        ...
        let roots = sqrt(magnitudes)
        
        var normalizedMagnitudes = [Float](count: N/2, repeatedValue: 0.0)
        vDSP_vsmul(roots, vDSP_Stride(1), [2.0 / Float(N)], &normalizedMagnitudes, 1, N2)
...
//        var fullPhases = phase
        ...
        let bandPassFilter = generateBandPassFilter(freqs)// Multiply phase and magnitude with the bandpass filter
    
        mag = mul(mag, y: bandPassFilter.0)
        phase = mul(phase, y: bandPassFilter.0)
    
        // Output Variables
        let filteredSpectrum = mul(normalizedMagnitudes, y: bandPassFilter.0)
