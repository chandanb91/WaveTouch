...
class LowPassFFT{
    ...
    private func generateLowPassFilter(freqs: [Float]) -> ([Float], Int, Int) {
        
        var minIdx = freqs.count+1
        var maxIdx = -1
        
        let lowPassFilter: [Float] = freqs.enumerate().map {
            (index, element) in
        ...
        for i in (0..<lowPassFilter.count) {
            
            if(lowPassFilter[i]==1.0) {
        ...
        return (lowPassFilter, minIdx, maxIdx)
    ...
    func calculate(_input: [Float]) -> [Float] {
        ...
        let lowPassFilter = generateLowPassFilter(freqs)// Multiply phase and magnitude with the bandpass filter
        
        mag = mul(mag, y: lowPassFilter.0)
        phase = mul(phase, y: lowPassFilter.0)
        
        // Output Variables
        let filteredSpectrum = mul(normalizedMagnitudes, y: lowPassFilter.0)
